...
class GiveawayButton(Button):
    def __init__(self, message_id: int, participant_count: int = 0):
        label = f"🎉 Join | {participant_count} Participant{'s' if participant_count != 1 else ''}"
        super().__init__(label=label, style=discord.ButtonStyle.green, custom_id=f"giveaway_{message_id}")
...
class GiveawayView(View):
    def __init__(self, message_id: int):
        ...
        participant_count = len(giveaways.get(message_id, {}).get("participants", []))
        self.add_item(GiveawayButton(message_id, participant_count))
