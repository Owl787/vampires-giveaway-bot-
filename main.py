...
class JoinButton(Button):
    def __init__(self, message_id: int, joined: bool = False):
        label = "❌ Leave" if joined else "🎉 Join"
        style = discord.ButtonStyle.red if joined else discord.ButtonStyle.green
        super().__init__(label=label, style=style, custom_id=f"join_{message_id}")
...
class ParticipantDisplayButton(Button):
    def __init__(self, participant_count: int):
        label = f"{participant_count} Participant{'s' if participant_count != 1 else ''}"
        super().__init__(label=label, style=discord.ButtonStyle.gray, disabled=True)

class GiveawayView(View):
    def __init__(self, message_id: int, user_id: int = None):
        super().__init__(timeout=None)
        giveaway = giveaways.get(message_id)
        participants = giveaway.get("participants", set()) if giveaway else set()
        joined = user_id in participants if user_id else False
        self.add_item(JoinButton(message_id, joined))
        self.add_item(ParticipantDisplayButton(len(participants)))
