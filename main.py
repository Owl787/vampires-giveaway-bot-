...
if not TOKEN: print("\u274c DISCORD_TOKEN is missing from your environment. Check your .env file.") exit()
...
async def callback(self, interaction: discord.Interaction):
    ...
    await giveaway["message"].edit(view=GiveawayView(self.message_id, user.id))
...
class GiveawayView(View): def init(self, message_id: int, user_id: int = None): super().init(timeout=None) giveaway = giveaways.get(message_id) participants = giveaway.get("participants", set()) if giveaway else set() joined = user_id in participants if user_id else False self.add_item(JoinButton(message_id, joined)) self.add_item(ParticipantDisplayButton(len(participants)))

@bot.event async def on_ready(): print(f"\u2705 Logged in as {bot.user}") for msg_id in giveaways: bot.add_view(GiveawayView(msg_id)) try: synced = await bot.tree.sync() print(f"\u2705 Synced {len(synced)} slash command(s).") except Exception as e: print(f"\u274c Failed to sync commands: {e}")
