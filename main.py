...
class JoinButton(Button): def init(self, message_id: int): super().init(label="Join", style=discord.ButtonStyle.green, custom_id=f"join_{message_id}") self.message_id = message_id
...
class ParticipantButton(Button): def init(self, message_id: int): count = len(giveaways.get(message_id, {}).get("participants", [])) super().init(label=f"Participants: {count}", style=discord.ButtonStyle.gray, disabled=True)

class GiveawayView(View): def init(self, message_id: int): super().init(timeout=None) self.add_item(JoinButton(message_id)) self.add_item(ParticipantButton(message_id))
...
if not TOKEN: print("DISCORD_TOKEN is missing from the environment. Make sure it's set in your .env file.") exit()
